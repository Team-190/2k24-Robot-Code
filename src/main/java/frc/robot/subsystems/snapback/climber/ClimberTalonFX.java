...
public class ClimberTalonFX implements ClimberIO{
    ...
    private final Alert leftDisconnectedAlert = new Alert("Left climber TalonFX disconnected, check CAN bus.",
            AlertType.ERROR);
    private final Alert rightDisconnectedAlert = new Alert("Right climber TalonFX disconnected, check CAN bus.",
            AlertType.ERROR);
    ...
    public ClimberTalonFX() {
        ...
        config.Slot0.kP = ClimberConstants.GAINS.kp();
        config.Slot0.kI = ClimberConstants.GAINS.ki();
        config.Slot0.kD = ClimberConstants.GAINS.kd();
        ...
        config.MotionMagic.MotionMagicAcceleration = ClimberConstants.MAX_ACCELERATION;
        ...
        BaseStatusSignal.setUpdateFrequencyForAll(50, leftPositionRotations, leftVelocityRotationsPerSecond,
                leftAppliedVolts, leftCurrentAmps, leftTemperatureCelsius, leftPositionGoalMeters, leftPositionErrorMeters, rightPositionRotations,
                rightVelocityRotationsPerSecond, rightAppliedVolts, rightCurrentAmps, rightTemperatureCelsius, rightPositionGoalMeters, rightPositionErrorMeters);
    }

    @Override
    public void updateInputs(ClimberIOInputs inputs) {
        boolean leftConnected = BaseStatusSignal.refreshAll(leftPositionRotations, leftVelocityRotationsPerSecond,
                leftAppliedVolts, leftCurrentAmps, leftTemperatureCelsius, leftPositionGoalMeters, leftPositionErrorMeters).isOK();
        boolean rightConnected = BaseStatusSignal.refreshAll(rightPositionRotations, rightVelocityRotationsPerSecond,
                rightAppliedVolts, rightCurrentAmps, rightTemperatureCelsius, rightPositionGoalMeters, rightPositionErrorMeters).isOK();

        leftDisconnectedAlert.set(!leftConnected);
        rightDisconnectedAlert.set(!rightConnected);

        inputs.leftPosition = leftPositionRotations.getValueAsDouble()*2*Math.PI*ClimberConstants.CLIMBER_PULLY_RADIUS_METERS;
        inputs.leftVelocityRadiansPerSecond = Units.rotationsToRadians(leftVelocityRotationsPerSecond.getValueAsDouble());
        ...
        inputs.rightPosition = rightPositionRotations.getValueAsDouble()*2*Math.PI*ClimberConstants.CLIMBER_PULLY_RADIUS_METERS;
        inputs.rightVelocityRadiansPerSecond = Units.rotationsToRadians(rightVelocityRotationsPerSecond.getValueAsDouble());
    ...
    public void setLeftPositionGoal(double positionMeters) {
        double positionRotations = positionMeters/(2*Math.PI*ClimberConstants.CLIMBER_PULLY_RADIUS_METERS);
        leftClimber.setControl(positionControlRequest.withPosition(positionRotations).withEnableFOC(true));
    }

    @Override
    public void setRightPositionGoal(double positionMeters) {
        double positionRotations = positionMeters/(2*Math.PI*ClimberConstants.CLIMBER_PULLY_RADIUS_METERS);
        rightClimber.setControl(positionControlRequest.withPosition(positionRotations).withEnableFOC(true));
    }
    ...
    public boolean atGoal() {
        double leftPositionMeters = leftPositionRotations.getValueAsDouble()*2*Math.PI*ClimberConstants.CLIMBER_PULLY_RADIUS_METERS;
        double rightPositionMeters = rightPositionRotations.getValueAsDouble()*2*Math.PI*ClimberConstants.CLIMBER_PULLY_RADIUS_METERS;

        return Math.abs(leftPositionMeters - leftPositionGoalMeters.getValueAsDouble()*2*Math.PI*ClimberConstants.CLIMBER_PULLY_RADIUS_METERS) < ClimberConstants.CLIMBER_TOLERANCE_METERS
                && Math.abs(rightPositionMeters - rightPositionGoalMeters.getValueAsDouble()*2*Math.PI*ClimberConstants.CLIMBER_PULLY_RADIUS_METERS) < ClimberConstants.CLIMBER_TOLERANCE_METERS;
    }
}
